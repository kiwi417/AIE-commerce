...
powershell -NoProfile -Command "$p = Get-CimInstance Win32_Process | Where-Object { $_.Name -eq 'node.exe' -and $_.CommandLine -match 'RuoYi-Vue-master' }; if ($p) { $p | ForEach-Object { Write-Host ('    kill vite PID ' + $_.ProcessId); Stop-Process -Id $_.ProcessId -Force } } else { Write-Host '    none found' }"
